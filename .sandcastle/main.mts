...
//                      createSandbox(). The implementer runs (up to 30
//                      iterations). All issue pipelines run concurrently
...
for (let iteration = 1; iteration <= MAX_ITERATIONS; iteration++) {
  ...
  const settled = await Promise.allSettled(
    issues.map(async (issue) => {
      ...
      try {
        // Run the implementer. Model picked by planner per-issue
        // (cost-routing); defaults to sonnet if planner omitted.
        // maxIterations capped at 30 — agents looping past this are
        // usually stuck and burning tokens. Let CI surface what broke.
